...
{
    cout << endl << "Path thread lunched" << endl;
   ...
   /* mutexPath.lock();
    ...
    {
        sf::sleep(sf::microseconds(1000/(Waiting->size()+1)));
...
void Path::PathThreadPr(Tinit &init)
{
   ...
   /* mutexPath.lock();
    ...
    while(init.pApp->isOpen())
    {
        sf::sleep(sf::microseconds(1000/(WaitingProche->size()+1)));
...
void Path::AddPathTask(Treaded &TreadingInfo)
{
    //cout << __FUNCTION__ << "- line " << __LINE__ << " : " << Waiting->size() << endl;
        ...
        //cout << "Waiting proche size : " << WaitingProche->size() << endl;
